...
static int __always_inline
dp_egr_main(void *ctx, struct xfrm *xf)
{
  if (xf->l2m.dl_type == ntohs(ETH_P_IP) && (xf->l34m.nw_proto == IPPROTO_TCP || xf->l34m.nw_proto == IPPROTO_UDP)) {
    ...
    key.saddr = xf->l34m.saddr4;
    key.daddr = xf->l34m.daddr4;
    key.sport = ntohs(xf->l34m.source);
    key.dport = ntohs(xf->l34m.dest);
    ...
    if (adat != NULL) {
      if (xf->pm.igr) {
        if (xf->l34m.nw_proto == IPPROTO_TCP) {
          dp_set_tcp_dst_ip(ctx, xf, adat->xaddr);
          dp_set_tcp_dport(ctx, xf, htons(adat->xport));
        } else if (xf->l34m.nw_proto == IPPROTO_UDP) {
          dp_set_udp_dst_ip(ctx, xf, adat->xaddr);
          dp_set_udp_dport(ctx, xf, htons(adat->xport));
        }
      } else if (xf->pm.egr) {
        if (xf->l34m.nw_proto == IPPROTO_TCP) {
          dp_set_tcp_src_ip(ctx, xf, adat->xaddr);
          dp_set_tcp_sport(ctx, xf, htons(adat->xport));
        } else if (xf->l34m.nw_proto == IPPROTO_UDP) {
          dp_set_udp_src_ip(ctx, xf, adat->xaddr);
          dp_set_udp_sport(ctx, xf, htons(adat->xport));
        }
      }
    }
  }
