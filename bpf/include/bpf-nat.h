...
dp_do_dnat(void *ctx, struct xpkt *pkt)
{
  ...
  if (pkt->l34.nw_proto == IPPROTO_TCP) {
    struct tcphdr *tcp = TC_PTR_ADD(FSM_PKT_DATA(ctx), pkt->pm.l4_off);
    if ((void *)(tcp + 1) > dend) {
    ...
    struct udphdr *udp = TC_PTR_ADD(FSM_PKT_DATA(ctx), pkt->pm.l4_off);
...
dp_do_snat(void *ctx, struct xpkt *pkt)
{
  ...
  if (pkt->l34.nw_proto == IPPROTO_TCP)  {
    struct iphdr *iph = TC_PTR(FSM_PKT_DATA(ctx) + pkt->pm.l3_off);
    if ((void *)(iph + 1) > dend)  {
