...
func (dc *MachineDiscoveryClient) CatalogServices(*connector.QueryOptions) ([]connector.NamespaceService, error) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	vms, err := dc.machineClient.MachineV1alpha1().VirtualMachines(dc.connectController.GetDeriveNamespace()).List(ctx, metav1.ListOptions{})
	if err != nil {
	...
	if len(vms.Items) > 0 {
		for _, vm := range vms.Items {
			...
			for _, svc := range vm.Spec.Services {
				catalogServices = append(catalogServices, connector.NamespaceService{Service: svc.ServiceName})
			}
		}
	}
...
func (dc *MachineDiscoveryClient) RegisteredServices(*connector.QueryOptions) ([]connector.NamespaceService, error) {
	// useless
	return nil, nil
}
