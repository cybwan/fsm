...
func (dc *NacosDiscoveryClient) CatalogServices(*connector.QueryOptions) ([]connector.NamespaceService, error) {
...
func (dc *NacosDiscoveryClient) RegisteredServices(*connector.QueryOptions) ([]connector.NamespaceService, error) {
	serviceList, err := dc.selectServices()
	if err != nil {
		return nil, err
	}
...
func (dc *NacosDiscoveryClient) Deregister(dereg *connector.CatalogDeregistration) error {
	...
	instanceId := dc.getServiceInstanceID(ins.ServiceName, ins.Ip, connector.MicroSvcPort(port), connector.ProtocolHTTP)
...
func (dc *NacosDiscoveryClient) getServiceInstanceID(name, addr string, port connector.MicroSvcPort, _ connector.MicroSvcProtocol) string {
	k2cGroupId := dc.connectController.GetNacosGroupId()
	if len(k2cGroupId) == 0 {
		k2cGroupId = constant.DEFAULT_GROUP
	}
