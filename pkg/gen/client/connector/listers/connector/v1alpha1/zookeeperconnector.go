...
func NewZookeeperConnectorLister(indexer cache.Indexer) ZookeeperConnectorLister {
	return &zookeeperConnectorLister{indexer: indexer}
}

// List lists all ZookeeperConnectors in the indexer.
func (s *zookeeperConnectorLister) List(selector labels.Selector) (ret []*v1alpha1.ZookeeperConnector, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ZookeeperConnector))
	})
	return ret, err
}

// Get retrieves the ZookeeperConnector from the index for a given name.
func (s *zookeeperConnectorLister) Get(name string) (*v1alpha1.ZookeeperConnector, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("zookeeperconnector"), name)
	}
	return obj.(*v1alpha1.ZookeeperConnector), nil
}
