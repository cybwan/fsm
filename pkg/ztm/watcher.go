...
func WatchMeshConfigUpdated(
	...
	for {
		...
		case event := <-meshCfgUpdateChan:
			...
			if prevObj.Spec.Ztm.LogLevel != newObj.Spec.Ztm.LogLevel {
				if err := logger.SetLogLevel(newObj.Spec.Ztm.LogLevel); err != nil {
					log.Error().Err(err).Msgf("Error setting controller log level to %s", newObj.Spec.Ztm.LogLevel)
				}
			}
			...
			if prevObj.Spec.ClusterSet.Name != newObj.Spec.ClusterSet.Name &&
				...
				agentController.SetClusterSet(newObj.Spec.ClusterSet.Name,
